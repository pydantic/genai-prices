...
from .build import runtime_unit_data
...
def package_data():
    provider_data, units = _load_wrapped_data(this_package_dir / 'data_v2.json')
...
'''
    ...
    runtime_units = runtime_unit_data(units)
    unit_data_content = f'''\
"""DO NOT EDIT THIS FILE DIRECTLY, INSTEAD RUN `make package-data`"""
...
def _load_wrapped_data(data_path: Path) -> tuple[list[JsonData], dict[str, Any]]:
    raw_payload = cast(object, json.loads(data_path.read_bytes()))
    if not isinstance(raw_payload, dict):
        raise ValueError(f'Expected {data_path} to contain exactly units and providers')
    payload = cast(dict[str, object], raw_payload)
    if set(payload) != {'units', 'providers'}:
        raise ValueError(f'Expected {data_path} to contain exactly units and providers')

    providers = payload['providers']
    units = payload['units']
    if not isinstance(providers, list):
        raise ValueError(f'Expected {data_path} providers to be an array')
    if not isinstance(units, dict):
        raise ValueError(f'Expected {data_path} units to be an object')

    return cast(list[JsonData], providers), cast(dict[str, Any], units)
