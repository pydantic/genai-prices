...
def load_v3_payload(data_path: Path) -> tuple[list[JsonData], RuntimeUnitProjection]:
    ...
    if not isinstance(raw_payload, dict):
        raise ValueError(f'Expected {data_path} to contain a v3 payload object')
    payload = cast(dict[str, object], raw_payload)

    providers = payload.get('providers')
    if not isinstance(providers, list):
        raise ValueError(f'Expected {data_path} v3 payload providers to be an array')

    raw_units = payload.get('units')
    if not isinstance(raw_units, dict):
        raise ValueError(f'Expected {data_path} v3 payload units to be an object')

    units = cast(RuntimeUnitProjection, raw_units)
    validate_runtime_unit_projection(units)
    return cast(list[JsonData], providers), units
...
def package_ts_data(provider_data: JsonData, units: RuntimeUnitProjection):
...
export const unitData: RawUnitsDict = {json.dumps(units, indent=2, ensure_ascii=False)};
"""
...
def package_go_data(provider_data: JsonData, units: RuntimeUnitProjection) -> None:
    ...
    for usage_key, unit in units.items():
        ...
        order_entries.append(f'\t{identifier},')
