...
def _iter_priced_key_sets(model: object) -> Iterator[tuple[str, set[str]]]:
    """Yield every set of price keys the engines can resolve, including each variant laid over the standard prices."""
    standard = [
        (label, _collect_model_price_keys(price)) for label, price in _iter_model_prices(getattr(model, 'prices'))
    ]
    yield from standard

    for index, variant in enumerate(cast(list[object], getattr(model, 'price_variants', None) or [])):
