...
def test_calc_unit_price_handles_inclusive_tier_boundary() -> None:
    ...
    assert calc_unit_price(price, 10, total_input_tokens=100, per=1_000, inclusive_tier_boundary=True) == Decimal(
        '0.02'
    )
...
def test_grok_4_6_long_context_cliff():
    ...
    """
    # The boundary is inclusive on xAI's side (">= 200k prompt tokens"), so it is pinned
    # from both directions: one token below stays on the base rate, while exactly 200k
    # is already on the higher one.
