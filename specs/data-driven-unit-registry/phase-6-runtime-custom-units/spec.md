# Phase 6: Runtime Custom Units

**Code-level architecture is in [code-spec](code-spec.md).**

**Phase 6 adds runtime unit editing on top of the optimized registry runtime.**
...
The public workflow is "edit the snapshot, then activate it if needed." Callers do not manually deep-copy the current registry, providers, models, or prices before making a small patch. The implementation uses copy-on-write detachment internally to provide validation rollback without exposing that ceremony to callers.

**Borrowed registry staging uses copy-on-write detachment.** _(from "Runtime unit and price patches happen through `DataSnapshot`")_
Earlier phases allow a `DataSnapshot` to borrow the active registry by default for backward compatibility. Phase 6 uses copy-on-write detachment on the first unit edit: the snapshot receives its own candidate registry copy before mutation, and failed edits roll back to the snapshot's previous registry. An edit to a staging snapshot never mutates the currently active registry by accident.
...
Registry mutations validate usage key uniqueness, price key uniqueness, reserved-name safety, dimension-set uniqueness, interval closure, and join-closedness before the mutation becomes visible. There is no externally visible invalid intermediate registry: mutation either commits a valid registry state or fails and leaves the registry unchanged.

**Existing-family unit edits are batch operations.** _(from "Registry mutations commit only structurally valid states", "Runtime custom units reuse the Phase 3 raw unit shape")_
...
Adding units without deleting or changing existing unit definitions must not force revalidation of every trusted built-in, fetched, or otherwise prevalidated price. Unchanged trusted prices remain trusted when the units and price-key mappings they reference still exist with the same meaning. Destructive changes can make affected prices stale, but validation should still be limited to changed, untrusted, or later-used affected prices rather than every price in the snapshot.

**Multiple executable snapshots remain out of scope.** _(from "Phase 6 adds runtime unit editing on top of the optimized registry runtime")_
Phase 6 adds runtime custom units inside the same active-global-snapshot assumption used by earlier phases. The guardrails that reject non-active snapshot execution are Phase 7. Supporting multiple concurrently executable snapshots would require a separate context/provenance design and is not part of this phase.
