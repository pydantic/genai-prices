...
The public workflow should be "edit the snapshot, then activate it if needed." Callers should not need to manually deep-copy the current registry, providers, models, or prices before making a small patch. The implementation may copy internally to provide validation rollback, but that staging machinery is not user-visible ceremony.

**Borrowed registry staging must be decided before mutation APIs land.** _(from "Runtime unit and price patches happen through `DataSnapshot`")_
Earlier phases allow a `DataSnapshot` to borrow the active registry by default for backward compatibility. Phase 6 must explicitly decide how first unit edits interact with a borrowed registry before adding APIs such as `DataSnapshot.add_units(...)`. Copy-on-write detachment, candidate registry transactions, explicit copied snapshots, or another rollback model are all acceptable if the final behavior is clear: a failed edit leaves the snapshot's previous registry visible and unchanged, and editing a staging snapshot must not accidentally mutate the active registry.
