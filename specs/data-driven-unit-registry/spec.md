...
Inferring a missing ancestor or overlap from reported descendants is useful but not required to expand the unit set. Phases 1 through 7 therefore do not synthesize missing registered usage values from descendants. They store reported values, use explicit values for reported reads and priced buckets, read tier thresholds through the same usage-read path, return zero for safely absent usage reads and priced buckets, and raise when a direct read or pricing calculation would require guessing an omitted ancestor or overlap. Safe missing reads do not materialize values or make a key count as reported. Phase 8 adds demand-driven inference as a separate reviewable feature.
...
The existing `TieredPrices` mechanism remains threshold-based: once a threshold is selected, that tier applies to the relevant unit count as it does today. Before Phase 8, the threshold input is `usage.input_tokens` read through the same missing-usage rules as ordinary access. If `input_tokens` was omitted and no positive related usage makes that unsafe, the threshold is zero and the base tier applies. If related reported usage means `input_tokens` would require inferring an omitted ancestor, the read raises instead of guessing a tier. Phase 8 can replace that conservative failure with registry-derived inference when the threshold is uniquely determined.
