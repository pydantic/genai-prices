...
**The registry defines all dimension combinations symmetrically.** _(from "Join coverage", "The registry is a YAML file")_
Every modality defines all four token slots (input, output, cache read, cache write) even where no provider currently uses them. This ensures the join of any two priced units always exists in the registry — join coverage validation can always fire. Models simply don't define prices for unused units.
...
**Naming optimizes for writability: people discover names by pattern-matching from existing ones.** _(from "The goal is to calculate prices as accurately as possible", "Backward compatibility")_
