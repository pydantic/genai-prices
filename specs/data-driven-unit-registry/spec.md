...
Once a conditional implication is part of a published v3 unit, a replacement representation must preserve that normalized meaning for the existing unit; [Phase 2](phase-2-auto-updating-units/spec.md) versions the semantics rather than the source syntax.
...
**The v1 provider artifacts are frozen compatibility snapshots.** _(from "Remote contracts are versioned instead of repurposed")_
Existing `prices/data.json`, `prices/data_slim.json`, and their schemas remain at the same URLs with their released provider-array structures and vocabularies. No build step rewrites them, and `tests/test_frozen_v1_data.py` pins their exact bytes. Provider, model, price, schema, and vocabulary changes belong in a versioned `prices/new_data/v<version>/` contract.
...
[Phase 2: Auto-Updating Unit Definitions](phase-2-auto-updating-units/spec.md) is a future independent change. It publishes `prices/new_data/v3/data.json` with units and providers, gives Python, JavaScript, and Go a paired v3 ingestion path, preserves v1/v2 URLs, and freezes the full and slim v2 payload/schema pairs at cutover. It is not required for the Phase 1 release. Its [code spec](phase-2-auto-updating-units/code-spec.md) derives the implementation architecture from that prose source of truth.
