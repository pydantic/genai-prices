...
The existing TieredPrices mechanism (threshold-based pricing where crossing a tier applies that rate to all tokens) continues to work as-is. The aggregate query concept generalizes TieredPrices' hardcoded `input_tokens` threshold into a dimension filter ({direction: input}), but TieredPrices is not modified to use it — that's a separate future change.
