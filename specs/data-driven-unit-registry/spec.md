...
The build rejects unknown usage keys, price keys, and extractor destinations before repository data or generated packages are published. An installed runtime may instead encounter data authored for a different registry version or retained by consumer code across an upgrade. Standard runtime usage construction, extraction, and pricing therefore warn about names that the active registry cannot use, ignore those names for standard calculations, and continue with recognized data. Unknown extractor mappings are skipped entirely, including their source-path requirements. This tolerance may produce a partial price, which the warning makes explicit; it does not weaken runtime errors for invalid values, incomplete ancestor or join prices among recognized units, missing required recognized usage, or contradictory recognized usage.
