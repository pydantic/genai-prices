...
A registry contains units keyed by usage key. Each unit has a price key, a normalization factor such as `per: 1_000_000`, and dimension assignments. The `family` dimension is required on every unit and replaces a separate family object: units with the same `dimensions.family` value can overlap, must have the same `per`, and are decomposed together. The runtime parses raw unit data into indexed `UnitDef` objects so pricing, validation, extraction, and display all use the same source of truth.
...
**The system is general across family dimension values.** _(from "The registry model is a data-defined unit graph used by every runtime")_
Any reported quantity with the shape `usage_value * price / normalization_factor` can be represented: tokens, requests, characters, duration, tool calls reported by an API, or future units. Tokens are the first complex family dimension value because their usage values overlap across direction, modality, and cache dimensions. The decomposition and validation model is not token-specific.

**The family dimension is the boundary for overlap.** _(from "The system is general across family dimension values")_
Usage values can overlap only within the same `dimensions.family` value. Token counts do not overlap request counts. Price calculation resolves each stored price key to exactly one usage-keyed unit, groups priced units by family dimension value, decomposes each group independently, and then aggregates costs.

**`requests_kcount` is an explicit one-request pricing exception.** _(from "The system is general across family dimension values")_
...
There is no separate declaration of allowed dimension keys or values. A unit with `{modality: video}` is how that family dimension value gains the `modality` axis and `video` value. This avoids a second declaration surface. The tradeoff is that a dimension typo can create a new axis or value; structural validation must then decide whether the resulting graph is usable. A stricter optional dimension schema can be added later if the typo protection becomes worth the extra authoring surface.
...
For each model, only the units with prices participate in decomposition. The runtime groups priced units by family dimension value, computes each priced unit's exclusive usage value from the dimension graph, multiplies by the stored price and the unit's `per`, and aggregates costs. Build/export validation requires all units with the same `dimensions.family` value to share the same `per`, so the normalization remains consistent across a decomposition group. The shared decomposition behavior is detailed in [algorithm](algorithm.md) and [examples](examples.md).
...
**Unit dimension sets are unique within a family dimension value.** _(from "Validation rules are semantic preconditions")_
Two units with the same `dimensions.family` value cannot have identical dimension assignments. The dimension set is the unit's position in the containment graph; duplicate dimension sets describe the same pricing bucket and are a data error.

**Ancestor coverage is required.** _(from "Validation rules are semantic preconditions")_
Pricing a specific unit requires pricing its ancestors with the same family dimension value. If a model prices `cache_read_tokens`, it must also price `input_tokens`; otherwise usage reported only at the ancestor level would have no price. This is validated before registry-driven pricing, not guessed at runtime.

**Join coverage is required for overlapping priced units.** _(from "Validation rules are semantic preconditions")_
If a model prices two compatible overlapping units with the same family dimension value, it must price their intersection too. Pricing both `cache_read_tokens` and `input_audio_tokens` requires pricing `cache_audio_read_tokens`. Without the join price, usage that is both cached and audio would be double-counted or misallocated.
...
For interval closure, if unit `A` is an ancestor of unit `B`, every dimension set formed by adding a non-empty proper subset of `B.dimensions - A.dimensions` to `A.dimensions` must also exist as a unit with that family dimension value. For join-closedness, two units are compatible when they have no conflicting value for the same dimension key; their join is the union of both dimension sets and must exist in the complete registry.
