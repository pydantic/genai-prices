...
**`set_custom_snapshot` validates the snapshot before activating it.** _(from "There is one global DataSnapshot", "Validation replaces what hardcoded fields gave us")_
When `set_custom_snapshot` is called, the snapshot's prices are validated against the snapshot's own unit families — ancestor coverage, join coverage, price key validity. If validation fails, the previous global snapshot remains in place. This is the moment where the complete picture exists: unit families and all model prices in one object. Validation always runs — there is no trust-bypass mechanism. For auto-updated pipeline data this is redundant but harmless (it runs in a background thread); for user-customized data it's essential.
...
Each unit defines a `usage_key` — the attribute name to look up on the usage object (e.g., `input_tokens` for unit `input_mtok`). The engine uses `getattr`/Mapping access. No typed Protocol enumerates usage fields. Callers provide whatever they have. The `Usage` class has no hardcoded fields — it uses dynamic attribute access backed by the registry, just like `ModelPrice`. Construction via keyword arguments (e.g., `Usage(input_tokens=1000, output_tokens=500)`) continues to work. Typed properties for common fields (e.g., `usage.input_tokens`) are a nice-to-have for autocomplete and type checking, low priority, and route through the same dynamic path. `Usage` supports keys from any family — token counts and request counts coexist on the same object. `Usage.__add__` (used for streaming accumulation) works with dynamic keys.
...
A caller with only `{input_tokens: 1000, output_tokens: 500}` gets a valid price at catch-all rates. A caller with only `{input_audio_tokens: 300}` gets a valid price at the audio rate — the catch-all ancestor's leaf value is zero (no remainder beyond what descendants account for), and its usage is inferred as 300. A caller with detailed breakdowns gets a more precise price. The system handles all of these by building leaf values bottom-up: missing usage at leaf-level priced units (no more-specific priced descendants) defaults to zero; missing usage at non-leaf priced units is inferred from descendants, giving the ancestor a leaf value of zero. Only explicitly provided values participate in the decomposition — the system prices exactly what was reported, nothing more. Conversely, if a caller provides a usage key (e.g., `input_video_tokens: 100`) for a model that doesn't price `input_video_mtok`, those tokens silently remain in the catch-all ancestor — the decomposition only considers priced units. Usage for a family with no priced units at all (e.g., `requests: 5` on a model with no request pricing) is silently ignored — there is nothing to decompose or price.
...
Incomplete and inconsistent are different. Missing usage is handled by inference (leaf defaults to zero, ancestors inferred from descendants). But when both a parent and child usage are explicitly provided and the child exceeds the parent — e.g., `{input_tokens: 100, cache_read_tokens: 200}` — that's contradictory: a subset can't exceed its superset. Decomposition detects this as a negative leaf value and raises an error. Negative leaves can only arise from explicitly contradictory provided values, never from inferred values. This is the right behavior — silently producing a wrong price is worse than failing loudly.
