...
Runtime code reads unit definitions from a single process-global registry, not from each `DataSnapshot`. Package startup builds that registry from generated unit data. After Phase 3, trusted remote update payloads can replace the global registry from their `unit_families` before provider data is installed. Provider snapshots do not carry custom or staged unit graphs.
