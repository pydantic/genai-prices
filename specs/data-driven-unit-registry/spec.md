...
[Phase 2: Auto-Updating Unit Definitions](phase-2-auto-updating-units/spec.md) is a future independent change. It publishes `prices/new_data/v3/data.json` with units and providers, gives Python, JavaScript, and Go a paired v3 ingestion path, preserves v1/v2 URLs, and freezes the full and slim v2 payload/schema pairs at cutover. It is not required for the Phase 1 release. Its existing [code spec](phase-2-auto-updating-units/code-spec.md) must be rewritten from the prose spec before implementation.
