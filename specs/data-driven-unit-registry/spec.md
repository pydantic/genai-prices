...
The built-in token family defines input, output, cache-read, cache-write, reasoning, and supported modality patterns consistently where those concepts make sense, while omitting nonsensical combinations such as output cache reads. Reasoning is an output property that may overlap a modality, so the built-in graph includes aggregate reasoning and its text, audio, image, and video joins. Arbitrary future families need only satisfy the structural definitions for the units they actually define.
