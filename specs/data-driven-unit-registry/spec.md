...
Repo tests must protect those built-in token naming conventions as data regressions. That test coverage is not runtime/export validation for arbitrary families; production validation still derives accepted keys and relationships from the registry data itself.
...
Families do not separately declare allowed dimension keys or values. A unit with `{modality: video}` is how that family gains the `modality` axis and `video` value. This avoids a second declaration surface. The tradeoff is that a dimension typo can create a new axis or value; structural validation must then decide whether the resulting graph is usable. A stricter optional dimension schema can be added later if the typo protection becomes worth the extra authoring surface.
...
Runtime clients can fetch updated prices before the next package release. A fetched price payload must not contain prices for units the client cannot parse. The long-term data contract therefore carries unit families in the same update payload as provider prices.

**There is one source registry and no standalone runtime units artifact.** _(from "Unit definitions must travel with prices")_
The checked-in source for built-in units is `prices/units.yml`. Runtime packages load generated language-native unit data at startup, and runtime auto-updates load unit families from the same fetched payload as providers after Phase 3. The design does not introduce a separate bundled `units.json` or another URL that could drift from prices.
