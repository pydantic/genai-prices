...
`prices/units.yml` defines usage-keyed units with a strictly positive integer normalization factor, an optional distinct price key, dimension assignments including a required `family` dimension, and any source-only information needed by publication validation. Generated language-native data carries only the runtime-semantic projection into both packages; handwritten runtime modules derive behavior through `UnitRegistry` rather than generated fields, build-only validation metadata, or hardcoded unit-name branches.
...
**Reported usage supports fractional quantities.** _(from "Price data is complete while usage data may be incomplete", "Phase 1 preserves supported consumer behavior unless accurate registry pricing makes that impossible")_
Recognized usage values are finite non-negative quantities rather than integer-only counts. Python stores `int | float | Decimal` values, preserving each supplied built-in numeric kind and normalizing accepted non-boolean `Integral` implementations to int for backward compatibility; JavaScript retains finite non-negative numbers. Python arithmetic converts integers exactly, interprets floats through their shortest round-trippable decimal spellings, and uses Decimals directly in a private Decimal context. Results promote to Decimal when any operand is Decimal, otherwise to float when any operand is float, otherwise remain int. The registry carries no discrete-versus-continuous metadata, tier starts remain integers, and fractional JSON-origin results require numerical rather than bit-identical cross-language parity.
...
Publication validates registry structure, public key safety, unique usage/price/dimension identities, interval closure, join-closedness, provider price-key and coverage rules, and extractor destinations. Build validation may use source-only metadata and intermediate representations that never enter generated packages. Runtime registry construction trusts the minimal generated package unit data and indexes it without repeating publication validation.
...
Construction can precede or bypass publication validation for custom objects, so standard base pricing validates the recognized selected-model prices against the bundled registry before decomposition. Unknown price names emit a runtime warning and are omitted from standard pricing; incomplete ancestor or join coverage among recognized prices remains an error. Phase 1 does not attach trust flags or validation markers to generated data.
...
Direct construction accepts externally reported bundled-registry keys with finite non-negative `int | float | Decimal` values. Supplied built-in values remain unchanged, so `3.0` remains float `3.0`; accepted non-boolean `Integral` implementations normalize to built-in int. Python booleans, negative values, non-finite floats and Decimals, and other numeric objects are invalid recognized usage. Unknown or pricing-only constructor names emit a runtime warning and remain ordinary attributes excluded from standard equality, addition, representation, missing reads, and base pricing. `Usage.from_raw(...)` reads known attributes from dataclasses, namespace objects, and other attribute objects while ignoring extras; it does not add plain-mapping input compatibility or warn for unrelated source attributes. The original object remains available to custom overrides. Assigning a registered key stores its value or removes it when assigned `None`; nonregistered names remain ordinary attributes. Integer-only addition returns integers; Decimal participation returns Decimal, and remaining addition involving floats returns float.
...
Registry-aware helpers read caller objects directly for pricing, while extractor output normalization retains externally reported bundled-registry usage keys. Open record types admit repo-defined keys. Standard pricing warns about unsupported usage and price keys and omits them, while invalid values attached to recognized keys and incomplete recognized price relationships remain errors.
...
Provider mappings may target reportable usage keys, including new modality keys, but not price keys, arbitrary names, or pricing-only `requests`. Build validation rejects invalid destinations. Runtime provider activation and direct extractor use warn instead, and extraction skips unsupported mappings without reading even a required source path. Extraction accumulates recognized reported counts but does not certify that a provider's related usage values are mutually consistent; contradictions remain subject to the explicit-only read and pricing rules.

Each recognized integer, float, or Decimal component is validated as finite and non-negative before accumulation. Invalid numeric components cannot be cancelled by other mappings to the same destination, and Python booleans cannot be laundered into integers. Optional path probing remains permissive for absent or structurally incompatible data, while a present supported numeric value outside the valid domain raises. Python accumulation uses the same numeric-domain promotion and shortest-decimal float interpretation as `Usage` addition.

Provider mappings report aggregate token categories when an API exposes only an aggregate. They additionally report a modality-specific category intersection only when the provider supplies that breakdown or documents the modality relationship; extraction does not invent a modality split. Existing aggregate input, output, and modality counts retain the provider-specific normalization required for accurate pricing.
...
`UsageExtractorMapping.dest` remains `str`, and direct `UsageExtractor` construction checks every mapping destination against the bundled registry. Unsupported destinations warn rather than preventing construction and are ignored by extraction.
...
**Deliberate v1 maintenance preserves released-client compatibility rather than byte identity.** _(from "The existing v1 JSON artifacts remain outside Phase 1 build output", "Phase 1 preserves supported consumer behavior")_
Existing package versions continue using the same v1 URLs and provider-array contract. The compatibility oracle for both payloads is `prices/data.schema.json` at target-main commit `ba8093719f296a3672ff4b2fc848a122e92a049c`, which describes the common provider-array input accepted by v1 package decoders. Each maintained payload must validate against that baseline schema. Exact artifact bytes may change through explicit compatible provider, model, and pricing maintenance. The current schema artifacts are not consumed by package auto-updaters and may evolve independently; they do not widen the v1 payload contract or permit new unit-related fields, unsupported price keys or extractor destinations, a wrapped root, or other data rejected by the baseline oracle.

**Phase 1 publishes full and slim provider-array v2 data.** _(from "Deliberate v1 maintenance preserves released-client compatibility rather than byte identity", "Pricing accuracy is the Phase 1 product outcome")_
The full `prices/new_data/v2/data.json` artifact keeps the same top-level provider-array shape as v1 but may contain prices and extractor destinations for every unit bundled with Phase 1. Its schema is published as `prices/new_data/v2/data.schema.json`. The generated `prices/new_data/v2/data_slim.json` projection and `prices/new_data/v2/data_slim.schema.json` exclude free models, provider `pricing_urls`, `description`, and `price_comments`, and model `name`, `description`, and `price_comments`. Neither form contains unit definitions or runtime performance state.

**Phase 1 auto-updaters use only the full v2 artifact.** _(from "Phase 1 publishes full and slim provider-array v2 data", "The Phase 1 registry is static for the lifetime of the installed package")_
...
**Tests prove the complete releasable contract.** _(from "Phase 1 must be independently shippable and releasable", "Deliberate v1 maintenance preserves released-client compatibility rather than byte identity")_
Coverage includes registry construction and strict publication validation; duplicate usage, price, and dimension identities; exact interval and join closure; public-name rejection; Python/JavaScript pricing parity; new modality, reasoning, reasoning-modality intersection, and image-token prices; explicit-only and contradictory usage behavior; runtime warnings and omission for unknown direct usage names, price keys, and extractor destinations; build-time rejection of unknown provider price keys and extractor destinations; custom Python pricing; flat and tiered raw-provider normalization; aggregate and modality-specific reasoning extractor destinations; built-in naming conventions; authoring schemas; CLI display of legacy and new registered prices; generated provider/unit separation and purity; the v1 provider-array contract; full and slim provider-array v2 generation; exact slim projection; no eager provider-activation model-price validation; v2 auto-update without registry replacement; activation despite unsupported extractor destinations; and alignment with the shared examples.
