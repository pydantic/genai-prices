...
CLI field discovery, labels, normalization display, and value formatting use stored price keys plus unit metadata. Existing CLI flags and output remain compatible for current units, and a repo-defined unit appears without a new hardcoded CLI branch.
