...
Recognized usage values are finite non-negative quantities rather than integer-only counts. Python stores built-in `int | float` values directly and preserves integer-only behavior; JavaScript retains finite non-negative numbers. Python operations involving floats temporarily use their shortest round-trippable decimal spellings in a private Decimal context and return floats, keeping usage JSON-native. The registry carries no discrete-versus-continuous metadata, tier starts remain integers, and fractional results require numerical rather than bit-identical cross-language parity.
...
Direct construction accepts externally reported bundled-registry keys with built-in finite non-negative `int | float` values. Integer inputs remain integers, float inputs remain floats, and Python booleans, Decimal values, negative values, and non-finite floats are invalid recognized usage. Unknown or pricing-only constructor names emit a runtime warning and remain ordinary attributes excluded from standard equality, addition, representation, missing reads, and base pricing. `Usage.from_raw(...)` reads known attributes from dataclasses, namespace objects, and other attribute objects while ignoring extras; it does not add plain-mapping input compatibility or warn for unrelated source attributes. The original object remains available to custom overrides. Assigning a registered key stores its value or removes it when assigned `None`; nonregistered names remain ordinary attributes. Integer-only addition returns integers; addition involving floats uses shortest-decimal temporary arithmetic and returns floats.
...
Each recognized integer or fractional numeric component is validated as finite and non-negative before accumulation. Invalid numeric components cannot be cancelled by other mappings to the same destination, and Python booleans cannot be laundered into integers. Optional path probing remains permissive for absent or structurally incompatible data, while a present numeric value outside the valid domain raises. Accumulation involving Python floats follows the same shortest-decimal rule as `Usage` addition.
