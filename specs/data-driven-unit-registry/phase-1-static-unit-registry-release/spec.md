...
Provider mappings report aggregate reasoning counts when an API exposes only an aggregate. They additionally report a modality-specific reasoning intersection only when the provider supplies that breakdown or documents the modality relationship; extraction does not invent a reasoning modality split. Existing aggregate output and modality counts retain the provider-specific normalization required for accurate pricing.
