...
Existing package versions continue using the same v1 URLs and provider-array contract. Exact artifact bytes may change through explicit compatible maintenance, including provider, model, pricing, or schema corrections, provided the resulting artifacts remain consumable by every released v1 client. A v1 data artifact must not gain a wrapped root, new unit-related fields, unsupported price keys or extractor destinations, or any other structure outside that contract; its schema must continue describing the v1 contract rather than the v2 vocabulary.
