...
Direct construction accepts externally reported bundled-registry keys and rejects unknown or pricing-only keys. `Usage.from_raw(...)` reads supported attributes and ignores extras for base pricing while the original object remains available to custom overrides. Assignment, equality, addition, representation, and missing reads stay live against the one bundled registry.
...
Base construction accepts candidate dynamic price-key keyword arguments and stores them as ordinary attributes. Registered old and new keys share construction, reads, validation, calculation, string display, and CLI behavior. Runtime provider parsing normalizes raw price mappings into this class before validating surrounding provider dataclasses.
...
Provider mappings may target reportable usage keys, including new modality keys, but not price keys, arbitrary names, or pricing-only `requests`. Build validation is authoritative; runtime provider activation checks destinations against the same static registry before replacing active provider data.
...
Generated editor schemas expose registry price keys and extractor destinations for autocomplete while build/export validation remains authoritative. Built-in token naming conventions are protected by repo-data regression tests rather than imposed as runtime laws for arbitrary future families.
...
**Performance evidence covers representative price shapes.** _(from "Phase 1 removes repeated structural work from the pricing hot path")_
Python and JavaScript benchmarks exercise direct and public pricing with empty, ordinary one/two-key, overlapping four-key, and complex five/six-key prices. Phase 1 uses those results to verify that the stateless cleanup removes the obvious regression; benchmarks do not introduce fragile absolute timing assertions into the test suite.

**Tests prove the complete releasable contract.** _(from "Phase 1 must be independently shippable and releasable")_
Coverage includes registry construction and publication validation, Python/JavaScript pricing parity, new modality and image-token prices, explicit-only usage behavior, dynamic price keys, custom Python pricing, extractor destinations, authoring schemas, CLI display, generated provider/unit separation, unchanged v1 artifacts, provider-array v2 generation, v2 auto-update without registry replacement, rollback of failed provider activation, and representative performance benchmarks.
