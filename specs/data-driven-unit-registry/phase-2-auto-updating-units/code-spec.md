...
remains unchanged. Add `tests/test_frozen_v2_data.py` to call the shared verifier. The existing lifecycle test continues
to assert the documented slim projection against the frozen full array. No runtime or generator receives a v2 write
helper after cutover.
...
Keep `UpdatePrices.fetch()` as the provider-data customization boundary. Add a focused subclass test whose override
calls `super().fetch()`, mutates each returned snapshot with a custom price and extractor, and returns that snapshot.
Exercise repeated calls without depending on private worker methods, thread ownership, or wait-result semantics. This
proves the private activation registry survives caller customization without adding an overlay store.

**Python background activation integrates without owning updater lifecycle.** _(implements "Python background activation installs the fetched pair after `fetch()` returns.", "Python updater coordination remains independent of Phase 2.", "Concurrent mutable-client operations gain no new consistency guarantee.")_
The updater's successful-refresh path calls side-effect-free `fetch()` and then `set_custom_snapshot(...)`, so a wrapped
snapshot activates providers and units together. Phase 2 makes no change to instance sharing, worker lifetime,
`start()`/`wait()`/`stop()` coordination, failure publication or retention, stop blocking, or post-stop active state.
Those semantics belong to the independent shared-updater implementation; Phase 2 does not copy them or add competing
tests. Passing `None` directly to `set_custom_snapshot(...)` remains the explicit complete-pair reset.
...
stale-target, and artifact-freeze coverage. Retain the existing lifecycle slim-projection test. Extend Python updater/provider-array/lifecycle/unit tests,
...
Python lazy custom prices and extractors across repeated fetches, paired updater activation, and JavaScript null/stale rejection. Add
