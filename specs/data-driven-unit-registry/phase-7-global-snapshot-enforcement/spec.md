...
The goal is to stop accidental execution against stale or inactive snapshot objects, not to make multiple snapshots safely executable at the same time. Supporting concurrent executable snapshots would require explicit context/provenance design across pricing, extraction, future usage inference, validation trust, decomposition caches, and runtime custom units. That is out of scope here.
