...
## Missing Usage Inference

Direct reads of reported values are deliberately simple: if `input_tokens` was
stored on the `Usage` object, `usage.input_tokens` returns that stored value.
The algorithm below applies only when a registered usage key is requested and
that key was not reported.

For a missing requested unit `U`, inference is the unique total implied by the
reported descendants of `U`. Conceptually:

1. Collect the stored strict descendants of `U` in the same family. Ignore
   reported units outside `U`'s subtree. If there are no reported descendants,
   return 0.
2. Build the atomic regions generated by those reported descendants. For each
   dimension key that appears below `U`, an atom records either one concrete
   reported value for that key or "other". Keep only atoms covered by at least
   one reported descendant; atoms outside all reports are treated as zero for
   this missing-read inference.
3. Create one non-negative variable per remaining atom.
4. For each reported descendant `D = value`, add an equation: the sum of atom
   variables contained in `D` equals `value`.
5. If the equations have no non-negative solution, raise a usage error because
   the reported descendants are contradictory.
6. Let `total(U)` be the sum of all remaining atom variables. If every
   non-negative solution gives the same `total(U)`, return that value.
7. Otherwise, raise a usage error because the requested value is
   underdetermined by the reported descendants.

This is a conceptual algorithm, not a requirement to ship a general linear
programming dependency. The built-in registry is small, and implementations can
use equivalent dimension/index logic. The important contract is the observable
behavior: return a stored value directly, infer a missing ancestor only when the
reported descendants determine one value, return zero when there is no relevant
descendant data, and raise when the missing value is contradictory or
underdetermined.

This creates one important distinction: an absent overlap is not always zero.
Reading `usage.cache_audio_read_tokens` directly with no reported cached-audio
descendants returns 0. But when inferring missing `input_tokens` from both
`input_audio_tokens` and `cache_read_tokens`, the cached-audio overlap is needed
to know the union of those two reported buckets. If that overlap is not reported
or otherwise determined by more-specific reports, `usage.input_tokens` raises
instead of assuming the overlap is zero.
...
Example: usage is `{input_audio_tokens: 300, cache_read_tokens: 200}` and
`input_tokens` is missing. The reported descendants overlap, but the overlap
`cache_audio_read_tokens` is unknown. The possible `input_tokens` totals range
from 300 to 500, so `usage.input_tokens` raises as underdetermined.

If the same usage also reports `cache_audio_read_tokens: 50`, the total is
determined:

```
usage.input_tokens = input_audio_tokens + cache_read_tokens - cache_audio_read_tokens
                   = 300 + 200 - 50
                   = 450
```

Example: usage is `{input_audio_tokens: 50, cache_audio_read_tokens: 100}` and
`input_tokens` is missing. Since cached-audio tokens are a subset of audio
tokens, the reported values cannot both be true. Inferring `input_tokens`
raises a contradiction error.

Example: usage is `{input_audio_tokens: 300, input_text_tokens: 200}` and
`input_tokens` is missing. The modality values conflict, so the reported
descendants do not overlap. The inferred total is 500.
