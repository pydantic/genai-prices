...
**Phase 1 proves the registry model in Python without changing public behavior.**
...
**The active registry is limited to the current Python unit surface.** _(from "Phase 1 proves the registry model in Python without changing public behavior")_
...
**The remote `data.json` and `data_slim.json` payloads remain provider arrays.** _(from "Phase 1 proves the registry model in Python without changing public behavior")_
Python can embed language-native unit registry data in generated package code, but the shared remote payload shape does not change until Phase 3. New repo-defined units are not an end-to-end feature until the shared payload carries unit families with prices.
...
**`Usage` becomes registry-aware and remains permissive for raw caller objects.** _(from "Phase 1 proves the registry model in Python without changing public behavior")_
...
**`ModelPrice` remains subclass-friendly and uses current legacy fields for storage.** _(from "Phase 1 proves the registry model in Python without changing public behavior")_
...
**`DataSnapshot` carries the Python registry but keeps current activation behavior.** _(from "The remote `data.json` and `data_slim.json` payloads remain provider arrays", "Validation protects pricing semantics without runtime trust caching")_
