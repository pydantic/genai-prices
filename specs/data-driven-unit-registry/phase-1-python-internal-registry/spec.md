...
First, raw registry data describes usage-keyed units, duplicated normalization factors, price keys, and unit dimensions including the required `family` dimension. Second, `UnitRegistry` parses that data into indexed `UnitDef` objects. Third, `Usage` stores and exposes reported usage keys without inferring omitted ancestors or overlaps. Fourth, base `ModelPrice.calc_price()` resolves price keys through the registry and decomposes only the units priced by the current model.
...
**Python uses one active global unit registry.** _(from "Phase 1 adds no separate runtime unit artifact")_
Phase 1 builds the active registry from generated package unit data and exposes it through a lazy module helper. `DataSnapshot` remains provider data only. Pricing, usage reads, extractor construction, and validation all consult the active global registry instead of a registry stored on the selected provider snapshot.

**`UnitRegistry` owns Python's runtime unit graph.** _(from "The active registry is limited to the current Python unit surface")_
The registry parses raw unit data, validates slice-appropriate structural integrity, and builds lookup indexes. It is a public read/lookup type from `genai_prices.units`, but Phase 1 does not expose public methods for mutating an existing registry.

**Python uses one runtime model per unit concept.** _(from "`UnitRegistry` owns Python's runtime unit graph")_
`UnitDef` is an identity dataclass populated by `UnitRegistry`, using `@dataclass(eq=False)` so unit objects remain stable identity handles for relationship checks. Python runtime code does not introduce separate Pydantic raw/parsed model layers for unit definitions. Raw registry data remains plain dictionaries until registry construction promotes usage-key dict keys into object fields and defaults missing `price_key` to the usage key.
...
Registry construction validates the unit structure. Price-level validation checks registered price keys, ancestor coverage, join coverage, and missing-join safety where needed. Ancestor and join validation receive the active `UnitRegistry` so they can use registry relationship indexes directly instead of duplicating scans. Runtime code always repeats one-model validation immediately before base `ModelPrice.calc_price()` uses a price. Phase 1 does not carry validation markers, price-key fingerprints, validation caches, or decomposition caches; those performance mechanisms belong to Phase 5.

**`calc_price` is a hot path and stays indexed.** _(from "Validation protects pricing semantics without runtime trust caching")_
Phase 1 should avoid full-snapshot validation, full-registry scans, or precomputed plan machinery in the normal request-pricing path. Runtime validation is one-model validation over the selected model's priced key set, performed every time standard base pricing calculates against that model price. Price-key-to-usage-key and usage-key-to-unit lookups use flat registry indexes, and each `UnitDef` carries its `per` normalization factor so normalization does not require another registry scan.

**`DataSnapshot` remains provider-only while the registry is global.** _(from "Python uses one active global unit registry", "Validation protects pricing semantics without runtime trust caching")_
The bundled provider snapshot is still built from generated provider data, while the bundled registry is built separately from generated package unit data. `set_custom_snapshot` does not validate model prices in Phase 1; it installs provider data, and any price-key, ancestor, join, or missing-join error is raised when standard base pricing first calculates against the affected `ModelPrice` using the active global registry.

**Inactive snapshots are provider staging objects.** _(from "`DataSnapshot` remains provider-only while the registry is global")_
Callers can fetch a provider snapshot, inspect providers and models, patch prices, and later activate it. Phase 1 does not try to make every inactive-snapshot execution path safe. Lookup-only helpers remain usable on inactive snapshots, and any pricing through those objects still reads the active global registry.

**Generated and fetched price feeds are trusted at the publication boundary.** _(from "`DataSnapshot` remains provider-only while the registry is global")_
Generated bundled provider and unit data are trusted because the build validated them. Fetched provider-array auto-update payloads remain trusted as prevalidated provider feeds in Phase 1. Phase 1 does not bulk-revalidate or selectively revalidate model prices during provider activation, and it does not serialize trust markers into generated data. Use-time validation is deliberately repeated until Phase 5 adds runtime-private global caches.
...
`requests` exists in the registry with `dimensions.family: requests` so `requests_kcount` participates in lookup, validation, display, and total-cost aggregation. It is not caller-supplied usage and not an extractor destination. Pricing code supplies one request per `Usage` object explicitly.
