...
`UnitDef` is a dataclass populated by `UnitRegistry`. Python runtime code does not introduce separate Pydantic raw/parsed model layers for unit definitions. Raw registry data remains plain dictionaries until registry construction promotes usage-key dict keys into object fields and defaults missing `price_key` to the usage key.
