...
Usage keys become usage attributes and extractor destinations. Price keys become model-price attributes and provider YAML keys. Registry-defined keys must therefore be public and attribute-safe in both Python and JavaScript before they can travel in wrapped runtime payloads. Build/export validation rejects obvious unsafe public names such as names that are not ASCII identifiers valid in both Python and JavaScript, private names, Python language keywords, and JavaScript language keywords.
