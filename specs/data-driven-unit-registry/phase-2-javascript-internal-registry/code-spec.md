...
Runtime update URLs still return provider arrays. Phase 2 therefore keeps update parsing compatible with the existing provider-array payload and preserves the active generated registry while replacing provider data. Local provider data can be parsed and structurally checked against the active parsed registry, but Phase 2 does not reject a provider update for model-price coverage; standard pricing validates the selected model price on use.
