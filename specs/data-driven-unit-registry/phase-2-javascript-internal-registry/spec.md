...
Callers continue passing plain objects. The runtime normalizes known externally reported usage keys into a plain object, ignores unknown extras, and keeps missing registered values missing. Contradictory usage can exist until price calculation must interpret it.
...
Direct reads of stored usage values are inert, missing values are not inferred in Phase 2, unpriced reported keys are ignored when explicit priced ancestors make them unnecessary, and request pricing uses the explicit one-request-per-usage-object rule in pricing code. Tier selection uses explicit `input_tokens`, matching Python's Phase 1 conservative rule.
