    ...
    Provider(
        ...
        models=[
            ...
            ModelInfo(
                ...
                context_window=500000,
                price_comments='Prompts at or above 200k tokens are billed at the higher rate for every token in the request, not just the tokens past the threshold. xAI\'s boundary is inclusive: its table columns read "< 200k prompt tokens" and ">= 200k prompt tokens", so the tier starts at the published 200000-token boundary. The base cache-read price is 0.30, not 0.50 - 0.50 is Grok 4.6\'s. Ref: https://docs.x.ai/docs/models/grok-4.5',
                prices=ModelPrice(
                    input_mtok=TieredPrices(base=Decimal('2'), tiers=[Tier(start=200000, price=Decimal('4'))]),
                    cache_read_mtok=TieredPrices(base=Decimal('0.3'), tiers=[Tier(start=200000, price=Decimal('0.6'))]),
            ...
            ModelInfo(
                ...
                prices=ModelPrice(
                    ...
                    cache_read_mtok=TieredPrices(base=Decimal('0.2'), tiers=[Tier(start=200000, price=Decimal('0.4'))]),
                    output_mtok=TieredPrices(base=Decimal('2'), tiers=[Tier(start=200000, price=Decimal('4'))]),
                ),
            ),
            ModelInfo(
                id='grok-code-fast-1',
