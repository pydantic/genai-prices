...
export function getProvidersSync(): Provider[] {
  try {
    ...
    if (!fs.existsSync(dataPath)) {
      ...
      dataPath = path.resolve(__dirname, '../../prices/data.json')
    }
    ...
    throw new Error(
      'No data available. This function requires either dist/data.json or access to the monorepo prices/data.json.',
    )
  }
}
