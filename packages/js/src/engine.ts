...
export function resolveModelWithFallback(
  ...
  const resolvedModel = resolveModelDirect(provider, modelId, allProviders)
  ...
  if (normalized !== modelId) return resolveModelDirect(provider, normalized, allProviders)
  return undefined
}

function resolveModelDirect(
  provider: Provider,
  modelId: string,
  allProviders?: Provider[]
): undefined | { model: ModelInfo; priceProvider: Provider } {
  ...
  if (provider.fallback_model_providers && allProviders) {
    for (const fallbackProviderId of provider.fallback_model_providers) {
      ...
      if (fallbackProvider) {
        ...
        const fallbackModel = resolveModelDirect(fallbackProvider, modelId)
