import type { ProviderDataValue } from '../../types'
...
updatePrices(({ onCalc, remoteDataUrl, setProviderData }) => {
  ...
  try {
    ...
    if (localStorageDataTimestamp !== null) {
      ...
      if (dataStr !== null) {
        setProviderData(JSON.parse(dataStr) as ProviderDataValue)
      }
  ...
  setProviderData(
    fetch(remoteDataUrl, { cache: 'no-store' }).then(async (response) => {
      const freshData = (await response.json()) as ProviderDataValue
