...
func findModel(providers []*provider, selected *provider, modelID string) (*model, error) {
	resolved, err := resolveModel(providers, selected, modelID)
	if resolved == nil {
		return nil, err
	}
...
func resolveModel(providers []*provider, selected *provider, modelID string) (*resolvedModel, error) {
	matched, err := resolveModelDirect(providers, selected, modelID, true)
	...
	return resolveModelDirect(providers, selected, normalized, true)
}

func resolveModelDirect(providers []*provider, selected *provider, modelID string, allowFallback bool) (*resolvedModel, error) {
	for index := range selected.Models {
		...
		if matched {
			return &resolvedModel{model: &selected.Models[index], priceProvider: selected}, nil
		}
	}
	if allowFallback {
		for _, fallbackID := range selected.FallbackModelProviders {
			for _, fallback := range providers {
				...
				matched, err := resolveModelDirect(providers, fallback, modelID, false)
				if err != nil || matched != nil {
					return matched, err
				}
			}
