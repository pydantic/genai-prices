...
func (calculator *Calculator) Calculate(request PriceRequest) (PriceCalculation, error) {
	...
	resolvedModel, err := resolveModel(calculator.providers, selected, modelID)
	if err != nil {
	...
	matchedModel := resolvedModel.model
	...
	inclusiveTierBoundary := resolvedModel.priceProvider.ID == "openai" || resolvedModel.priceProvider.ID == "x-ai"
	inputPrice, outputPrice, totalPrice, err := calculateModelPrice(
		usage,
		prices,
		calculator.registry,
		inclusiveTierBoundary,
	)
	if err != nil {
		return PriceCalculation{}, err
	}
