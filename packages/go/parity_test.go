...
func TestProviderTierBoundaryConventions(t *testing.T) {
	calculator := newTestCalculator(t, `[
        {
            "id":"openai","name":"OpenAI","api_pattern":"openai",
            "models":[
                {"id":"openai-model","match":{"equals":"openai-model"},"prices":{"input_mtok":{"base":1,"tiers":[{"start":100,"price":2}]}}}
            ]
        },
        {
            "id":"x-ai","name":"xAI","api_pattern":"x-ai",
            "models":[
                {"id":"xai-model","match":{"equals":"xai-model"},"prices":{"input_mtok":{"base":1,"tiers":[{"start":100,"price":2}]}}}
            ]
        },
        {
            "id":"anthropic","name":"Anthropic","api_pattern":"anthropic",
            "models":[
                {"id":"anthropic-model","match":{"equals":"anthropic-model"},"prices":{"input_mtok":{"base":1,"tiers":[{"start":100,"price":2}]}}}
            ]
        },
        {
            "id":"azure","name":"Azure","api_pattern":"azure","fallback_model_providers":["openai"],"models":[]
        }
    ]`)
	...
	tests := []struct {
		...
		{providerID: "openai", model: "openai-model", want: 0.0002},
		{providerID: "x-ai", model: "xai-model", want: 0.0002},
		{providerID: "anthropic", model: "anthropic-model", want: 0.0001},
		{providerID: "azure", model: "openai-model", want: 0.0002},
	}
	for _, test := range tests {
		t.Run(test.providerID, func(t *testing.T) {
			calculation, err := calculator.Calculate(genai_prices.PriceRequest{
				ProviderID: test.providerID,
				Model:      test.model,
				Usage:      genai_prices.Usage{genai_prices.UsageInputTokens: 100},
			})
			if err != nil {
				t.Fatal(err)
			}
			if calculation.TotalPrice != test.want {
				t.Fatalf("got %g, want %g", calculation.TotalPrice, test.want)
			}
		})
	}
}
