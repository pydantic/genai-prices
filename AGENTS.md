...
- Python tests use pytest and are in `tests/`; JS tests are in `packages/js/src/__tests__/` and run
  via `npm run ci`
- CI and local `make test` both enforce **100% coverage** on `packages/python/**` and `tests/**`
  (omitting `tests/dataset/**`), and a **52% ratchet** on `prices/src/**` and `tests/dataset/**`. Raise
  the pipeline number as it improves; never lower it.
