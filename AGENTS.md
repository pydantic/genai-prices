...
- **Never hand-edit `tests/dataset/usages.json` or add synthetic feature cases to it.** It is the
  ...
  synthetic behavior belongs in focused Python and JavaScript tests. `make test` rewrites the generated
  file and then fails if legitimate recorded inputs changed its output, in which case inspect and commit
  the generated diff. Because it writes before comparing, a second `make test` always passes; don't read
  that as a fix.
